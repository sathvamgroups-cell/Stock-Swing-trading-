...
Original file is located at
    https://colab.research.google.com/drive/1WB06-t83CDEkSQw9pG4BUaW5YQzp4ynN
...
tickers = [
    "DMART.NS","NAUKRI.NS","TATACONSUM.NS","TCS.NS","HDFCBANK.NS", "ICICIBANK.NS", "AXISBANK.NS", "BEL.NS", "DLF.NS", "GODREJCP.NS",
    "HDFCLIFE.NS","MAXHEALTH","SBILIFE","TRENT.NS","VBL.NS","MGL.NS",
    "KPIL","GMRTHU","RELIANCE.NS", "TATAMOTORS.NS", "MARUTI.NS","SWIGGY","ETERNAL","ITC.NS","PAYTM","ADANPOWER","HEG","BPCL.NS",
]
...
for ticker in tickers:
    print(f"Running backtest on {ticker}")
    data = yf.download(ticker, start="2023-10-01", end="2024-12-31")
    if data.empty:
        print(f"No data for {ticker}, skipping.")
        continue
    if isinstance(data.columns, pd.MultiIndex):
...
tickers = [
    "DMART.NS", "NAUKRI.NS", "TATACONSUM.NS", "TCS.NS", "HDFCBANK.NS",
    "ICICIBANK.NS", "AXISBANK.NS", "BEL.NS", "DLF.NS", "GODREJCP.NS",
    "HDFCLIFE.NS", "SBILIFE.NS", "TRENT.NS", "VBL.NS", "MGL.NS",
    "RELIANCE.NS", "TATAMOTORS.NS", "MARUTI.NS", "ITC.NS", "HEG.NS",
    "BPCL.NS", "BAJFINANCE.NS", "LTI.NS", "BANKBARODA.NS", "PIDILITIND.NS"
]
...
for ticker in tickers:
    print(f"Running backtest on {ticker}")
    data = yf.download(ticker, start="2022-10-01", end="2023-12-31")
    if data.empty:
        print(f"No data for {ticker}, skipping.")
        continue
    if isinstance(data.columns, pd.MultiIndex):
...
for ticker in tickers:
    print(f"Running backtest on {ticker}")
    data = yf.download(ticker, start="2021-10-01", end="2022-12-31")
    if data.empty:
        print(f"No data for {ticker}, skipping.")
        continue
    if isinstance(data.columns, pd.MultiIndex):
...
tickers = [
    "DMART.NS", "TCS.NS", "HDFCBANK.NS", "RELIANCE.NS", "ITC.NS",
    "SBILIFE.NS", "BAJFINANCE.NS", "DLF.NS", "TRENT.NS", "GODREJCP.NS",
    "MGL.NS", "BEL.NS", "VBL.NS", "HDFCLIFE.NS", "NAUKRI.NS"
]
...
for ticker in tickers:
    print(f"Running backtest on {ticker}")
    data = yf.download(ticker, start="2020-10-01", end="2021-12-31")
    if data.empty:
        print(f"No data for {ticker}, skipping.")
        continue
    if isinstance(data.columns, pd.MultiIndex):
...
tickers = [
    "TCS.NS", "RELIANCE.NS", "ITC.NS", "SBILIFE.NS", "HDFCBANK.NS",
    "BAJFINANCE.NS", "DLF.NS", "TRENT.NS", "GODREJCP.NS", "MGL.NS",
    "BEL.NS", "VBL.NS", "HDFCLIFE.NS", "NAUKRI.NS", "ICICIBANK.NS"
]
...
tickers = [
    "TCS.NS", "RELIANCE.NS", "HDFCBANK.NS", "ICICIBANK.NS", "MARUTI.NS",
    "BAJFINANCE.NS", "DLF.NS", "TRENT.NS", "GODREJCP.NS", "SBILIFE.NS",
    "MGL.NS", "BEL.NS", "VBL.NS", "HDFCLIFE.NS", "NAUKRI.NS"
]
...
class CustomStrategy(bt.Strategy):
    params = dict(rsi_period=14, ma_params=(10, 20, 50), vol_period=20)

    def __init__(self):
        self.rsi = bt.indicators.RSI(self.data.close, period=self.p.rsi_period)
        self.ma10 = bt.indicators.SMA(self.data.close, period=self.p.ma_params[0])
        self.ma20 = bt.indicators.SMA(self.data.close, period=self.p.ma_params[1])
        self.ma50 = bt.indicators.SMA(self.data.close, period=self.p.ma_params[2])
        self.volma = bt.indicators.SMA(self.data.volume, period=self.p.vol_period)
        self.trade_records = []

    def next(self):
        if len(self) < max(self.p.ma_params):
            return
    ...
    def notify_trade(self, trade):
        if trade.isclosed:
            open_dt = trade.open_datetime() if callable(trade.open_datetime) else trade.open_datetime
            close_dt = trade.close_datetime() if callable(trade.close_datetime) else trade.close_datetime
...
for ticker in tickers:
    print(f'Running backtest for {ticker}')
    data = yf.download(ticker, start='2018-10-01', end='2019-12-31')
...
class CustomStrategy(bt.Strategy):
    ...
    def next(self):
        if len(self) < max(self.p.ma_periods):
            return
...
for ticker in tickers:
    ...
    net_profit = trade_analyzer.get('pnl', {}).get('net', {}).get('total', 0)

    print(f"{ticker}: Total trades = {total_trades}, Wins = {won_trades}, Losses = {lost_trades}, Net profit = {net_profit}")
    ...
    results.append({
        ...
        'Sharpe Ratio': strat.analyzers.sharpe.get_analysis().get('sharperatio'),
        'SQN': strat.analyzers.sqn.get_analysis().get('sqn'),
...
for ticker in tickers:
    try:
        data = yf.download(ticker, start="2019-10-01", end="2020-12-31")
    except Exception as e:
        print(f"Error downloading {ticker}: {e}")
        continue
